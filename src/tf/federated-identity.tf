...
resource "aws_iam_openid_connect_provider" "lz_k3s" {
  url = "https://k8s-oidc-lz.levizitting.com"
...
resource "aws_iam_policy" "lz_k3s_kubernetes_workload_boundary" {
  name        = "LZK3sKubernetesWorkloadBoundary"
  path        = "/lz-k3s/"
  description = "Require LZ K3s workload roles to be used only by Kubernetes service accounts"
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Condition = {
          ArnNotEquals = {
            "aws:FederatedProvider" = aws_iam_openid_connect_provider.lz_k3s.arn
          }
        }
      },
      {
        ...
        Condition = {
          StringNotLike = {
            "k8s-oidc-lz.levizitting.com:sub" = "system:serviceaccount:*:*"
          }
        }
      },
      {
        ...
        Condition = {
          StringNotEquals = {
            "k8s-oidc-lz.levizitting.com:aud" = "sts.amazonaws.com"
          }
        }
      },
      {
